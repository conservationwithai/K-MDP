function [PK, RK, S2K, K2S, PolicyK, PolKs, error, time, time_KMDP] = aStarKMDP(K, precision, P, R, discount, V, Policy)

%aStarKMDP State abstraction \phi_{a^*_d} K-MDP algorithm
% Arguments -----------------------------------------------
...
%V(V==0) = -100;
    ...
    while p > precision
          ...
          %Compute the ceil of all values in V
          for s=1:NS
              bindings(s,1) = ceil(V(s)/d);
              bindings(s,2) = Policy(s);
          end    
